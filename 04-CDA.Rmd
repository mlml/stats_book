...
<!-- TODO FUTURE: move all solutions to end of file, since code folding doesn't work -->
...
```{r odds1, echo=FALSE, fig.align='center', fig.height=3, fig.width=3, fig.cap = 'Odds as a function of probability.'}
...
```{r logodds, echo=FALSE, fig.align='center', fig.height=3, fig.width=3, fig.cap = 'Log-odds as a function of probability'}
...
```{r oddsratio, echo=FALSE, warning=FALSE, fig.align='center', fig.height=3, fig.width=3.5, fig.cap="Odds ratio when a probability ($p$) is increased by 0.1, as a function of $p$"}
p <- seq(0.001, 0.899, by = 0.01)
oddsRat <- ((p+0.1)/(1-p-0.1))/(p/(1-p))

qplot(p, oddsRat, geom = "line") +  
  xlab("Probability") + 
  ylab("Odds ratio when p -> p+0.1") + xlim(0,1) + ylim(1.4,4)
```
...
<div class="fold o">
```{r, echo=FALSE}
log(0.90/(1-0.90)) - log(0.70/(1-0.70))
```
</div>
